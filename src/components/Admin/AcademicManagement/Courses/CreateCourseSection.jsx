...
import {createSection} from "../../../../lib/api/Course.jsx";
...
export default function CreateCourseSection() {
    ...
    async function handleSubmit(e) {
        e.preventDefault()
        const response = await createSection(token,{id, name})
        console.log(response.status)
        if(response.status === 201) {
         await  alertSuccess("Create Section Berhasil")
            setName('')
        }else {
          await alertError("Create Section Gagal, Cek Kembali Data Anda, back end no info")
        }
    }
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="text" id="facultyName" name="facultyName" value={name} onChange={(e) => setName(e.target.value)}
                                       ...
                                       placeholder="Just Fill A, B or C" required/>
                            </div>
                        </div>
                    </div>
